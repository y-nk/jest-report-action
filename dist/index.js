...
var markdown_table_exports = {};
__export(markdown_table_exports, {
...
var { markdownTable: markdownTable2 } = (init_markdown_table(), markdown_table_exports);
var REPORT_FILE = join(process.cwd(), "report.json");
var JEST_OPTIONS = ["--json", "--coverage", `--outputFile=${REPORT_FILE}`];
var main = async () => {
  const labels = context.payload && context.payload.pull_request && context.payload.pull_request.labels && context.payload.pull_request.labels.map(({ name }) => name);
  const skip_on = core.getInput("skip_on");
  if (skip_on && labels.indexOf(skip_on) !== -1)
    return;
  const cwd = process.cwd();
  const bin = core.getInput("jest", { required: false });
  const cmd = ["npm", "npx"].some((e) => bin.startsWith(e)) ? [bin, "--", ...JEST_OPTIONS] : [bin, ...JEST_OPTIONS];
  try {
  ...
  if (!report.success) {
    const messages = report.testResults.filter((testResult) => testResult.status === "failed").map((testResult) => testResult.message);
  ...
  const body = [
    ...
    markdownTable2(rows, { align: ["l", "r", "r", "r", "r"] })
  ].join("\n\n");
